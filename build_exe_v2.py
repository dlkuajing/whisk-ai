...
'''
    ...
    with open('whisk_v2.spec', 'w', encoding='utf-8') as f:
...
def create_distribution_package():
    """创建完整的分发包"""
    if not Path('dist/WhiskAI_V2.exe').exists():
    ...
    # 创建分发目录
    dist_dir = Path('WhiskAI_V2_Distribution')
    if dist_dir.exists():
        shutil.rmtree(dist_dir)
    dist_dir.mkdir()
    
    # 复制exe文件
    shutil.copy2('dist/WhiskAI_V2.exe', dist_dir / 'WhiskAI_V2.exe')
    ...
    # 创建启动脚本
    start_script = dist_dir / '启动Whisk.bat'
...
### V2.1版本特性（最新）
- 窗口串行队列功能 - 同一窗口任务自动排队执行
- 任务组（套图）归档管理 - 相关任务统一文件夹
- 三种归档模式 - 独立/套图/每次询问
- 浏览器连接复用 - 提升执行效率
- 动态任务添加 - 运行中随时添加新任务
...
def main():
    """主函数"""
    print("=" * 60)
    print("Whisk AI V2.1 Windows EXE 打包工具")
    print("=" * 60)
    ...
    print("[SUCCESS] 打包完成！")
    print("=" * 60)
    print(f"输出目录: WhiskAI_V2_Distribution/")
    print(f"主程序: WhiskAI_V2_Distribution/WhiskAI_V2.exe")
    print(f"使用说明: WhiskAI_V2_Distribution/README.md")
    print("=" * 60)
    
    # 显示文件大小信息
    exe_path = Path('WhiskAI_V2_Distribution/WhiskAI_V2.exe')
    if exe_path.exists():
        size_mb = exe_path.stat().st_size / (1024 * 1024)
        print(f"文件大小: {size_mb:.2f} MB")
    
    print("\n[SUCCESS] 现在您可以将 WhiskAI_V2_Distribution 文件夹复制到其他Windows电脑上使用！")
    input("\n按回车键退出...")
